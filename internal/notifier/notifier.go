...
import (
	...
	"gitlab.com/navyx/ai/maos/maos-core/internal/baseservice"
...
type notifierTestSignals struct {
	BackoffError   testhelper.TestSignal[error]    // non-cancellation error received by main run loop
	ListeningBegin testhelper.TestSignal[struct{}] // notifier has entered a listen loop
	ListeningEnd   testhelper.TestSignal[struct{}] // notifier has left a listen loop
}
...
type Notifier struct {
	...
	notificationBuf   chan *listener.Notification
	statusChangeFunc  func(startstop.Status)
...
func New(logger *slog.Logger, li listener.Listener, statusChangeFunc func(startstop.Status)) *Notifier {
	notifier := baseservice.Init(logger, &Notifier{
		listener:          li,
		notificationBuf:   make(chan *listener.Notification, 1000),
...
func (n *Notifier) Start(ctx context.Context) error {
	...
	go func() {
		...
		n.statusChangeFunc(startstop.Initializing)
		...
		n.statusChangeFunc(startstop.ShuttingDown)
		wg.Wait()
		n.statusChangeFunc(startstop.Stopped)
	}()
	...
	for {
		...
		case notification := <-n.notificationBuf:
			notifyFuncs := func() []NotifyFunc {
				...
				return lo.Map(n.subscriptions[NotificationTopic(notification.Topic)], func(s *Subscription, _ int) NotifyFunc { return s.notifyFunc })
			}()
...
func (n *Notifier) listenAndWait(ctx context.Context) error {
	...
	topics := func() []NotificationTopic {
		...
		return lo.Keys(n.subscriptions)
	}()
	...
	n.statusChangeFunc(startstop.Healthy)
	...
	for {
		...
		if err != nil {
			...
			n.statusChangeFunc(startstop.Unhealthy)
