...
func TestStress(t *testing.T) {
	...
	Stress(ctx, t, &MyService{logger: testhelper.Logger(t)})
}
...
func TestStressErr(t *testing.T) {
	...
	StressErr(ctx, t, &MyService{logger: testhelper.Logger(t), startErr: startErr}, startErr)

	mockT := newMockTestingT(t)
	StressErr(ctx, mockT, &MyService{logger: testhelper.Logger(t), startErr: errors.New("different error")}, startErr)
