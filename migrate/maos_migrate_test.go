...
var (
	// Test migrations are based on the actual migrations.
	// Get their maximum version number to define test versions, preventing test breakage when adding new migrations.
...
func TestMigrator(t *testing.T) {
	...
	setup := func(t *testing.T) (*migrate.Migrator, *testBundle) {
		...
		migrator := migrate.New(dbPool, bundle.logger)
	...
	t.Run("MigrateDownDefault", func(t *testing.T) {
		...
		_, err = bundle.dbPool.Exec(ctx, "SELECT name FROM test_table")
		require.Error(t, err)
	...
	t.Run("MigrateDownWithMaxSteps", func(t *testing.T) {
		...
		migrations, err := querier.MigrationGetAll(ctx, bundle.dbPool)
		require.NoError(t, err)
	...
	t.Run("MigrateDownWithTargetVersionMinusOne", func(t *testing.T) {
		...
		_, err = bundle.dbPool.Exec(ctx, "SELECT name FROM _migrate")
		require.Error(t, err)
	})
