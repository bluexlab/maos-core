...
func TestInvocationGetNextEndpoint(t *testing.T) {
	...
	t.Run("Invocations exist", func(t *testing.T) {
		...
		resp := GetHttp(t, server.URL+"/v1/invocations/next", token.ID)
	...
	t.Run("Timeout", func(t *testing.T) {
		...
		resp := GetHttp(t, server.URL+"/v1/invocations/next?wait=1", token.ID)
		require.Equal(t, http.StatusNotFound, resp.StatusCode)

		_, err := testhelper.ReadBody(resp.Body)
		require.NoError(t, err)
	})
	...
	t.Run("Invocations insert after", func(t *testing.T) {
		...
		go func() {
			...
			resp := PostHttp(t, server.URL+"/v1/invocations/async", body, userToken.ID)
